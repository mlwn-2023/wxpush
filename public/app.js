...
function renderRecipientChecks() { const root = $('#recipientChecks'); root.innerHTML = ''; state.recipients.filter(r => r.enabled).forEach(r => { const label = document.createElement('label'); label.className = 'check-card'; const input = document.createElement('input'); input.type = 'checkbox'; input.value = r.id; input.dataset.group = r.group_name; input.addEventListener('change', updateRecipientGroupActions); const div = document.createElement('div'); const b = document.createElement('b'); b.textContent = r.name; const small = document.createElement('small'); small.textContent = r.group_name; div.append(b, small); label.append(input, div); root.append(label); }); renderRecipientGroupActions(); toggleChecks(); }
function renderRecipientGroupActions() { const root=$('#recipientGroupActions'); root.innerHTML=''; const recipients=state.recipients.filter(r=>r.enabled); if(!recipients.length){root.hidden=true;return;} root.hidden=false; root.append(recipientActionButton('全选',()=>setRecipientChecks(true),'utility'),recipientActionButton('清空',()=>setRecipientChecks(false),'utility')); const groups=[...new Set(recipients.map(r=>r.group_name||'默认分组'))].sort((a,b)=>a.localeCompare(b,'zh-CN')); groups.forEach(group=>{const button=recipientActionButton(group,()=>toggleRecipientGroup(group),'group');button.dataset.group=group;root.append(button);}); updateRecipientGroupActions(); }
function recipientActionButton(label,handler,type){const button=document.createElement('button');button.type='button';button.className=`group-select-btn ${type}`;button.dataset.label=label;button.textContent=label;button.addEventListener('click',handler);return button;}
function setRecipientChecks(checked){$$('#recipientChecks input:not(:disabled)').forEach(input=>input.checked=checked);updateRecipientGroupActions();}
function toggleRecipientGroup(group){const inputs=$$('#recipientChecks input').filter(input=>input.dataset.group===group&&!input.disabled);const select=!inputs.length?false:!inputs.every(input=>input.checked);inputs.forEach(input=>input.checked=select);updateRecipientGroupActions();}
function updateRecipientGroupActions(){const allMode=$('#sendAll').checked;const root=$('#recipientGroupActions');root.classList.toggle('disabled',allMode);$$('.group-select-btn',root).forEach(button=>{button.disabled=allMode;if(button.dataset.group===undefined)return;const inputs=$$('#recipientChecks input').filter(input=>input.dataset.group===button.dataset.group);const selected=inputs.filter(input=>input.checked).length;button.textContent=`${button.dataset.label} ${selected}/${inputs.length}`;button.classList.toggle('selected',selected===inputs.length&&inputs.length>0);button.classList.toggle('partial',selected>0&&selected<inputs.length);});}
