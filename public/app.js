...
async function loadTemplates(){const data=await api('/api/templates');state.templates=data.templates;renderTemplates();const select=$('#composeTemplate');const current=select.value;select.innerHTML='<option value="">不使用模板</option>';state.templates.forEach(t=>{const o=document.createElement('option');o.value=t.id;o.textContent=t.name;select.append(o);});select.value=current;}
