...
export function createStore(filename) {
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS api_signatures (signature_hash TEXT PRIMARY KEY,expires_at INTEGER NOT NULL,created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP);
  ...
  ensureColumn(db, 'messages', 'wechat_template_id', 'TEXT');
  ensureColumn(db, 'messages', 'wechat_data', "TEXT NOT NULL DEFAULT '{}'");
  ensureColumn(db, 'message_templates', 'wechat_template_id', 'TEXT');
  ensureColumn(db, 'message_templates', 'wechat_data', "TEXT NOT NULL DEFAULT '{}'");
  ensureColumn(db, 'schedules', 'wechat_template_id', 'TEXT');
  ensureColumn(db, 'schedules', 'wechat_data', "TEXT NOT NULL DEFAULT '{}'");
  ...
  const q = {
    ...
    addMessage: db.prepare(`INSERT INTO messages(public_id,title,content,recipients,recipient_count,success_count,status,error,source,attempts,wechat_template_id,wechat_data) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)`),
    ...
    addApiSignature: db.prepare('INSERT INTO api_signatures(signature_hash,expires_at) VALUES(?,?)'), cleanupApiSignatures: db.prepare('DELETE FROM api_signatures WHERE expires_at<=?'),
    listSchedules: db.prepare('SELECT * FROM schedules ORDER BY enabled DESC,next_run_at ASC,id DESC'), getSchedule: db.prepare('SELECT * FROM schedules WHERE id=?'), addSchedule: db.prepare('INSERT INTO schedules(name,title,content,recipient_ids,send_all,recurrence,next_run_at,enabled,wechat_template_id,wechat_data) VALUES(?,?,?,?,?,?,?,?,?,?)'), updateSchedule: db.prepare('UPDATE schedules SET name=?,title=?,content=?,recipient_ids=?,send_all=?,recurrence=?,next_run_at=?,enabled=?,wechat_template_id=?,wechat_data=?,updated_at=CURRENT_TIMESTAMP WHERE id=?'), deleteSchedule: db.prepare('DELETE FROM schedules WHERE id=?'), dueSchedules: db.prepare("SELECT * FROM schedules WHERE enabled=1 AND next_run_at<=? ORDER BY next_run_at LIMIT 20"), updateScheduleRun: db.prepare('UPDATE schedules SET enabled=?,next_run_at=?,last_run_at=CURRENT_TIMESTAMP,last_status=?,last_error=?,updated_at=CURRENT_TIMESTAMP WHERE id=?')
  };

  const decodeMessage = m => m ? { ...m, recipients: safeJson(m.recipients, []), wechat_data: safeJson(m.wechat_data, {}) } : null;
  const decodeTemplate = t => t ? { ...t, wechat_data: safeJson(t.wechat_data, {}) } : null;
  const decodeSchedule = s => s ? { ...s, recipient_ids: safeJson(s.recipient_ids, []), wechat_data: safeJson(s.wechat_data, {}), send_all: Boolean(s.send_all), enabled: Boolean(s.enabled) } : null;
  return {
    db, filename,
    ...
    listTemplates(){return q.listTemplates.all().map(decodeTemplate);},getTemplate(id){return decodeTemplate(q.getTemplate.get(id));},addTemplate(v){return Number(q.addTemplate.run(v.name,v.title,v.content,v.wechatTemplateId||null,JSON.stringify(v.wechatData||{})).lastInsertRowid);},updateTemplate(id,v){return q.updateTemplate.run(v.name,v.title,v.content,v.wechatTemplateId||null,JSON.stringify(v.wechatData||{}),id).changes;},deleteTemplate(id){return q.deleteTemplate.run(id).changes;},
    ...
    rememberApiSignature(hash,expiresAt,now){q.cleanupApiSignatures.run(now);try{q.addApiSignature.run(hash,expiresAt);return true;}catch(error){if([1555,2067].includes(Number(error.errcode))||String(error.errstr||'').includes('constraint'))return false;throw error;}},
    listSchedules(){return q.listSchedules.all().map(decodeSchedule);},getSchedule(id){return decodeSchedule(q.getSchedule.get(id));},addSchedule(v){return Number(q.addSchedule.run(v.name,v.title,v.content,JSON.stringify(v.recipientIds),v.sendAll?1:0,v.recurrence,v.nextRunAt,v.enabled===false?0:1,v.wechatTemplateId||null,JSON.stringify(v.wechatData||{})).lastInsertRowid);},updateSchedule(id,v){return q.updateSchedule.run(v.name,v.title,v.content,JSON.stringify(v.recipientIds),v.sendAll?1:0,v.recurrence,v.nextRunAt,v.enabled===false?0:1,v.wechatTemplateId||null,JSON.stringify(v.wechatData||{}),id).changes;},deleteSchedule(id){return q.deleteSchedule.run(id).changes;},dueSchedules(now){return q.dueSchedules.all(now).map(decodeSchedule);},updateScheduleRun(id,v){q.updateScheduleRun.run(v.enabled?1:0,v.nextRunAt,v.status,v.error||null,id);},
